...
export function ZoneCalibration({ intersection, language }: ZoneCalibrationProps) {
  ...
  const ptzPresets = ptzPresetsFromRedux.length > 0 ? ptzPresetsFromRedux : [];

  const getDirectionLabel = (dir: 'north' | 'south' | 'east' | 'west') => {
    return language === 'fa'
      ? { north: 'شمال', south: 'جنوب', east: 'شرق', west: 'غرب' }[dir]
      : { north: 'North', south: 'South', east: 'East', west: 'West' }[dir];
  };

  const availableViews: View[] = [
    ...fixedDirections.map(dir => ({
      id: dir,
      label: getDirectionLabel(dir),
      type: 'fixed' as const,
      direction: dir,
    })),
  ...
  const [showDirectionMasks] = useState(true);
  const [showViolationMasks] = useState(true);
  ...
  const saveEditedName = () => {
    ...
    setShapes(shapes.map(s =>
      s.id === editingShapeId ? { ...s, name: editedName.trim() || (language === 'fa' ? 'منطقه بدون نام' : 'Unnamed Zone') } : s
    ));
    setEditingShapeId(null);
  ...
  useEffect(() => {
    ...
    const loadedShapes: Shape[] = existingMasks.map(mask => {
      ...
      if (mask.ptzPresetId) viewId = mask.ptzPresetId;
      else if (mask.direction) viewId = mask.direction;
  ...
  const getName = () => {
    if (calibrationStep === 'direction') {
      const view = availableViews.find(v => v.id === selectedViewId);
      return `${language === 'fa' ? 'منطقه اصلی' : 'Main Zone'} (${view?.label || '—'})`;
    }
    const vt = violationTypes.find(v => v.id === selectedViolationType);
    return vt?.name || (language === 'fa' ? 'تخلف' : 'Violation');
  };
  ...
  const handleCanvasClick = (e: React.MouseEvent<HTMLCanvasElement>) => {
    const canvas = canvasRef.current;
    if (!canvas || !selectedViewId) return;
    ...
    if (activeTool === 'select') {
      ...
      if (!isDrawing) {
        ...
        setShapes(updated);
        setIsDrawing(false);
        toast.success(t.rectangleCreated);
      }
  ...
  const handleCanvasDoubleClick = () => {
    if (activeTool === 'polygon' && isDrawing && selectedShapeId) {
      ...
      if (shape && shape.points.length >= 3) {
        setIsDrawing(false);
        setSelectedShapeId(null);
        toast.success(t.polygonCompleted);
      } else {
      ...
      <div className="max-w-[1800px] mx-auto">
        <div className="mb-8">
          <h2 className="text-xl font-bold text-slate-900 dark:text-slate-100 mb-4">
            {t.zoneCalibrationTitle} — {intersection.name}
          </h2>
          <p className="text-sm text-slate-600 dark:text-slate-400 mt-1">
            {t.zoneCalibrationDesc}
          </p>
        </div>
          ...
          <Card className="flex flex-col border border-slate-200 dark:border-slate-700 shadow-sm bg-white dark:bg-slate-800 rounded-lg overflow-hidden">
            <div className="flex items-center justify-between p-3">
              ...
              <div className="flex gap-1.5">
                <Button variant={activeTool === 'select' ? 'default' : 'outline'} size="sm" className="text-[10px] h-6 px-2" onClick={() => setActiveTool('select')}>
                  <MousePointer2 className="w-3 h-3 mr-1" /> {t.selectTool}
                </Button>
                <Button variant={activeTool === 'rectangle' ? 'default' : 'outline'} size="sm" className="text-[10px] h-6 px-2" onClick={() => setActiveTool('rectangle')}>
                  {t.rectangleTool}
                </Button>
                <Button variant={activeTool === 'polygon' ? 'default' : 'outline'} size="sm" className="text-[10px] h-6 px-2" onClick={() => setActiveTool('polygon')}>
                  {t.polygonTool}
                </Button>
              </div>
            </div>
            ...
            {selectedViewId ? (
              ...
              <div className="flex items-center justify-center flex-1 bg-slate-100 dark:bg-slate-800 rounded border border-dashed border-slate-300 dark:border-slate-600">
                <p className="text-slate-500 dark:text-slate-400 text-sm">{t.selectViewFirst}</p>
              </div>
            )}
          </Card>
          ...
          <div className="flex flex-col gap-4 h-full">
            <Card className="shadow-lg p-3 border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 rounded-lg flex flex-col" style={{ height: 'calc(50% - 8px)' }}>
              ...
              {ptzPresets.length > 0 ? (
                ...
                <p className="text-[10px] text-slate-500 dark:text-slate-400">{t.noPtzPreset}</p>
              )}
            </Card>
