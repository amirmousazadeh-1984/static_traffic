...
export function IntersectionDashboard({ intersection }: IntersectionDashboardProps) {
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      case 'verified':
        return <Badge className="bg-green-100 dark:bg-green-900/30 text-green-800 dark:text-green-300 text-xs">تخلفات تایید شده</Badge>;
      case 'pending':
        return <Badge className="bg-amber-100 dark:bg-amber-900/30 text-amber-800 dark:text-amber-300 text-xs">تخلفات در انتظار</Badge>;
      case 'rejected':
        return <Badge className="bg-red-100 dark:bg-red-900/30 text-red-800 dark:text-red-300 text-xs">تخلفات رد شده</Badge>;
      default:
        return null;
    }
  };
      ...
      <div className="max-w-[1800px] mx-auto px-6 py-8">
        ...
        <div className="flex items-center justify-between mb-8">
          <div>
            ...
            <p className="text-sm text-slate-600 dark:text-slate-400 mt-1">نظارت زنده</p>
          </div>
          <div className="flex items-center gap-4">
            <div className="flex items-center gap-2">
              <div className={`w-2.5 h-2.5 rounded-full ${liveMonitoring ? 'bg-green-500' : 'bg-slate-400'}`} />
              <span className="text-sm text-slate-700 dark:text-slate-300">{liveMonitoring ? 'فعال' : 'متوقف'}</span>
            </div>
            <Button
              size="sm"
        ...
        <div className="grid grid-cols-2 md:grid-cols-5 gap-5 mb-8">
          <Card className="p-5 border border-slate-200 dark:border-slate-700 shadow-sm hover:shadow transition-shadow bg-white dark:bg-slate-800">
            <p className="text-xs text-slate-600 dark:text-slate-400 uppercase tracking-wider">کل تخلفات</p>
            <p className="text-2xl font-bold text-slate-900 dark:text-slate-100 mt-2">{stats.total}</p>
          </Card>

          <Card className="p-5 border border-slate-200 dark:border-slate-700 shadow-sm hover:shadow transition-shadow bg-white dark:bg-slate-800">
            <p className="text-xs text-green-700 dark:text-green-300 uppercase tracking-wider">تخلفات تایید شده</p>
            <p className="text-2xl font-bold text-green-900 dark:text-green-100 mt-2">{stats.byStatus.verified}</p>
          </Card>

          <Card className="p-5 border border-slate-200 dark:border-slate-700 shadow-sm hover:shadow transition-shadow bg-white dark:bg-slate-800">
            <p className="text-xs text-amber-700 dark:text-amber-300 uppercase tracking-wider">تخلفات در انتظار</p>
            <p className="text-2xl font-bold text-amber-900 dark:text-amber-100 mt-2">{stats.byStatus.pending}</p>
          </Card>

          <Card className="p-5 border border-slate-200 dark:border-slate-700 shadow-sm hover:shadow transition-shadow bg-white dark:bg-slate-800">
            <p className="text-xs text-red-700 dark:text-red-300 uppercase tracking-wider">تخلفات رد شده</p>
            <p className="text-2xl font-bold text-red-900 dark:text-red-100 mt-2">{stats.byStatus.rejected}</p>
          </Card>

          <Card className="p-5 border border-slate-200 dark:border-slate-700 shadow-sm hover:shadow transition-shadow bg-white dark:bg-slate-800">
            <p className="text-xs text-purple-700 dark:text-purple-300 uppercase tracking-wider">حالت دوربین چرخان</p>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <Card className="p-6 border border-slate-200 dark:border-slate-700 shadow-sm bg-white dark:bg-slate-800">
              <h3 className="text-base font-semibold text-slate-900 dark:text-slate-100 mb-4">دوربین‌های ثابت</h3>
              ...
              <div className="grid grid-cols-2 gap-4">
                {cameras.filter(c => c.type === 'fixed').length === 0 ? (
                  <p className="col-span-2 text-center text-slate-500 dark:text-slate-400 py-6">دوربین ثابتی موجود نیست</p>
                ) : (
                  cameras.filter(c => c.type === 'fixed').map((camera) => (
                    <div key={camera.id} className="relative bg-slate-100 dark:bg-slate-800/50 rounded-xl aspect-video flex items-center justify-center border border-slate-300 dark:border-slate-600 hover:border-slate-400 dark:hover:border-slate-500 transition-colors">
            ...
            <Card className="p-6 border border-slate-200 dark:border-slate-700 shadow-sm bg-white dark:bg-slate-800">
              <div className="flex items-center justify-between mb-4">
                <h3 className="text-base font-semibold text-slate-900 dark:text-slate-100">دوربین PTZ</h3>
                <Button
                  size="sm"
            ...
            <Card className="p-6 border border-slate-200 dark:border-slate-700 shadow-sm bg-white dark:bg-slate-800">
              <h3 className="text-base font-semibold text-slate-900 dark:text-slate-100 mb-5">تخلفات به تفکیک نوع</h3>
                  ...
                  return (
                    <div key={type} className="space-y-2">
                      <div className="flex items-center justify-between text-sm">
                        <span className="text-slate-700 dark:text-slate-300">{getViolationTypeLabel(type)}</span>
              ...
              <Tabs defaultValue="all" className="w-full">
                ...
                <TabsContent value="all" className="space-y-4 max-h-96 overflow-y-auto">
                  {recentViolations.length === 0 ? (
                    <p className="text-center text-slate-500 dark:text-slate-400 py-8 text-sm">تخلفی ثبت نشده</p>
                  ) : (
                      ...
                      >
                        <div className="flex items-center gap-3 mb-3">
                          ...
                          <div className="flex-1">
                            <p className="font-semibold text-sm text-slate-900 dark:text-slate-100">{violation.plateNumber}</p>
                            <p className="text-xs text-slate-600 dark:text-slate-400 mt-1">{getViolationTypeLabel(violation.violationType)}</p>
                ...
                <TabsContent value="verified" className="space-y-4 max-h-96 overflow-y-auto">
                  {recentViolations.filter(v => v.status === 'verified').map((violation) => (
                    <div key={violation.id} className="p-4 bg-green-50 dark:bg-green-900/20 rounded-lg border border-green-200 dark:border-green-800">
                      <div className="flex items-center gap-3 mb-3">
                ...
                <TabsContent value="pending" className="space-y-4 max-h-96 overflow-y-auto">
                  {recentViolations.filter(v => v.status === 'pending').map((violation) => (
                    <div key={violation.id} className="p-4 bg-amber-50 dark:bg-amber-900/20 rounded-lg border border-amber-200 dark:border-amber-800">
                      <div className="flex items-center gap-3 mb-3">
